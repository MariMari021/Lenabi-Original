...
// MOBILE
document.addEventListener("DOMContentLoaded", function () {
    const carrinhoItensMobile = JSON.parse(localStorage.getItem("carrinho")) || [];
    const itensListaMobile = document.getElementById("itens-mobile");
    const precoTotalSpanMobile = document.getElementById("preco-totalMobile");
    const limparCarrinhoBtnMobile = document.querySelector(".carMobile");
    const contFavoritosSpanMobile = document.getElementById("cont-favoritos");




    function limparCarrinhoMobile() {
        carrinhoItensMobile.length = 0;
        atualizarCarrinhoMobile();
        salvarCarrinhoNoLocalStorageMobile();
        resetProdutinhosMobile();
    ...
    function atualizarCarrinhoMobile() {
        ...
        carrinhoItensMobile.forEach(item => {
            ...
            liMobile.innerHTML = `
                <div class="item-carrinho">
                    ...
                    <span class="quantity">${item.quantidade}</span>
                    <button class="add" onclick="addMaisUm('${item.nome}')">+</button>
                    <span class="preco-totalMobile">R$${(item.preco * item.quantidade).toFixed(2)}</span>
                </div>
            `;
            itensListaMobile.appendChild(liMobile);
            total += item.preco * item.quantidade;
        });

        precoTotalSpanMobile.textContent = `Valor Total: R$${total.toFixed(2)}`;
        contFavoritosSpanMobile.textContent = carrinhoItensMobile.reduce((acc, item) => acc + item.quantidade, 0).toString();
        limparCarrinhoBtnMobile.style.display = "block";

    }

    function addToCartMobile(nome, preco) {
        const existingItem = carrinhoItensMobile.find(item => item.nome === nome);

        if (existingItem) {
            existingItem.quantidade++;
        } else {
            carrinhoItensMobile.push({ nome, preco, quantidade: 1 });
        }
    ...
    window.removeCarrinhoMobile = function (nome) {
        const index = carrinhoItensMobile.findIndex(item => item.nome === nome);

        if (index !== -1) {
            const item = carrinhoItensMobile[index];
            item.quantidade--;
            ...
            if (item.quantidade === 0) {
                ...
                carrinhoItensMobile.splice(index, 1);
            }
        }
    ...
    window.addMaisUmMobile = function (nome) {
        const item = carrinhoItensMobile.find(item => item.nome === nome);
    ...
    function resetProdutinhosMobile() {
        const produtinhosBtnsMobile = document.querySelectorAll(".produtinhos");
        produtinhosBtnsMobile.forEach(btn => {
            const coracaoVazio = btn.querySelector(".icon-coracao");
            const coracaoPintado = btn.querySelector(".icon-coracaoP");
            const valorInicialElement = btn.nextElementSibling; // assume que p.espec-coracao é irmão do botão
            const produtoNome = btn.getAttribute("data-produto-nome");
            const produtoNoCarrinho = carrinhoItensMobile.find(item => item.nome === produtoNome);
    ...
    produtinhoBtns.forEach(produtinhoBtn => {
        produtinhoBtn.addEventListener("click", function () {
            ...
            if (!produtoNoCarrinho) {
                addToCartMobile(nome, preco);
            } else {
                removeCarrinhoMobile(nome);
            }
    ...
    limparCarrinhoBtnMobile.addEventListener("click", function () {
        limparCarrinhoMobile();
    });

    function salvarCarrinhoNoLocalStorageMobile() {
        localStorage.setItem("carrinho", JSON.stringify(carrinhoItens));
    }
